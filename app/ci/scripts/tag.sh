...
if ! git describe --tag --match "ui-v*" --exact-match $GIT_COMMIT 2>&1 | grep "fatal";
then
  git describe --tag --match "ui-v*" --abbrev=0 --exact-match $GIT_COMMIT > ../$REPO_NAME/tag.txt
  echo "Commit is already tagged, using tag: $(cat ../$REPO_NAME/tag.txt)"
  exit 0
fi

echo "Getting previous tag with ui- prefix"
CURRENT_TAG=$(git describe --tag --match "ui-v*" --abbrev=0 2>/dev/null)
if [ "${CURRENT_TAG}" = "" ]
then
  echo "No ui- prefixed tags found, looking for v* tags as fallback"
...
then
  ...
  NEW_TAG="ui-v${MAJOR}.${MINOR}.${PATCH}"
fi
